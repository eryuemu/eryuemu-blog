...
## 一、现象：三封 GSC 报警邮件的时间线全景

回顾近期站长邮箱收到的 Google Search Console 邮件，完整的时间线与事件脉络如下：

![8 月 17 日博客首次收到 GSC 邮件——提示「全站网页」因「网页会自动重定向」未编入索引（前篇 8/18 改造的起因）](../../assets/gsc-email-blog-redirect-alert-aug17.png)
*图 1：8 月 17 日博客首次收到 GSC 邮件——提示「全站网页」因「网页会自动重定向」未编入索引（前篇 8/18 改造的起因）*

![8 月 20 日博客第 2 次收到 GSC 邮件——提示「站点地图中的网页」存在重定向（8/18 提交验证后的过渡期提醒）](../../assets/gsc-email-blog-redirect-alert.png)
*图 2：8 月 20 日博客第 2 次收到 GSC 邮件——提示「站点地图中的网页」存在重定向（8/18 提交验证后的过渡期提醒）*
...
其实对比这三封邮件的标题与日期，就能清晰看懂背后的业务逻辑：
1. **8 月 17 日博客首封邮件（图 1）**：标题为 *「网站 https://eryuemu.com/ 【上的网页】无法被编入索引」*。这是 Google 全站爬虫在 7 月底~8 月中旬抓取到 308 重定向后触发的首次汇总，**正是这封信促使我们在 8 月 18 日将博客从 www 反转为主域名 `eryuemu.com` 并提交了 GSC 验证**；
2. **8 月 20 日博客第 2 封邮件（图 2）**：标题细化为 *「网站 https://eryuemu.com/ 的【站点地图中的网页】无法被编入索引」*。这是 Google 爬虫在处理 `sitemap-index.xml` 报表维度时触发的提醒。因为 8 月 18 日提交的验证还在队列中跑着，系统按例行机制发送了站点地图维度的账单；
3. **8 月 20 日知识库首封邮件（图 3）**：这是知识库副站（`guide.hbuwiki.top`）在上线后**第一次**收到邮件，触发了全新的路径规范化场景（「备用网页」）。

带着这两个站点的不同线索，我们分别登录两边的 GSC 控制台后台展开全量剖析。
