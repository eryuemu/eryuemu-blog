...
pubDate: '2026-07-20T16:54:28+08:00'
category: '开发'
type: 'ai-organized'
---
...
**问题的根源是域名而不是 IP：`*.vercel.app` 后缀在国内被 DNS 污染，任何 `.vercel.app` 域名都打不开；换成自己的独立域名后，DNS 解析干净，请求正常到达 Vercel。至于用哪条 A 记录（`76.76.21.21` 还是 Vercel 面板推荐的 `216.198.79.1`），实测两个 IP 国内均可直连，不是关键因素。**
...
**IP 从来不是问题——Vercel 的 A 记录 IP（`76.76.21.21`、`216.198.79.1` 等）国内均可直连（2026-08 实测）。**
当你用自己的独立域名 `eryuemu.com` 访问时，DNS 解析正常返回 Vercel 的 IP，请求能到达 Vercel 边缘；而 `.vercel.app` 在解析环节就被污染，根本拿不到真实 IP。差别从头到尾只在"域名能不能解析"这一环。

> 通俗总结：GFW 在门口查身份证（域名），不是查你的房子地址（IP）。你的 `.vercel.app` 身份证进了黑名单，但 `eryuemu.com` 这张新身份证是干净的，同一个房子（服务器）随便进。

### 5.3 澄清：网上流传的"推荐 IP 被墙"说法并不成立（2026-08 实测）

当时网上流传"Vercel 面板推荐的 `216.198.79.1` 被 GFW 屏蔽、必须用经典 IP `76.76.21.21`"。2026-08 在国内网络直连实测：

| IP | 归属 | TCP 443 | TLS 握手 |
|----|------|---------|----------|
| `216.198.79.1` | 亚马逊 AWS（AS16509） | ✅ 可连 | ✅ 成功 |
| `76.76.21.21` | 亚马逊 AWS（AS16509） | ✅ 可连 | ✅ 成功 |

两个 IP 其实都是 Vercel 租用的 AWS 地址，国内均可直连，"新 IP 被墙"是**想当然的脑补**。真正决定能否访问的从头到尾只有一个：**域名**。`*.vercel.app` 被 DNS 污染，自定义域名干净，仅此而已。Vercel 面板提示的加速 DNS 配置（`216.198.79.1` / `cname-2.vercel-dns.com`）只要解析正常，同样可用。

### 5.4 当前配置：自定义域名 + Vercel DNS 记录
...
这套配置在国内可正常访问，本质原因不是"经典 IP 更安全"，而是**自定义域名绕开了 `.vercel.app` 的 DNS 污染**。

### 5.5 真正的敌人：域名级 DNS 污染

`*.vercel.app` 被 GFW 在 DNS 解析环节整体污染（返回假 IP / 超时），所以国内访问任何 `.vercel.app` 域名都打不开。换成自己的域名后，DNS 解析干净，请求正常到达 Vercel 边缘——**同一个服务，换个干净的域名入口即可访问**。

> Vercel 面板上的黄色警告 "DNS Change Recommended" 仅仅是升级提示，**不代表当前配置会失效**。可以永久忽略它。
...
在 Spaceship 将 A 记录从 `216.198.79.1` 换回 `76.76.21.21` 后，手机依然打不开。
...
- 你的手机本地 DNS 缓存着"旧 IP"
- 你家的路由器也缓存着"旧 IP"
- 运营商的递归 DNS 服务器同样缓存着"旧 IP"

三层缓存叠在一起，即使你在 Spaceship 后台改了记录，终端设备依然向旧 IP 发请求 → 依然打不开。
