...
6.4 把现象归因于 Antigravity 处理"最近文件追踪"时的误伤，当时尚属孤例；如今第二台电脑出现**完全相同的改动痕迹**（`Start_TrackProgs` 归零 + `RunMRU` 清空）。本机没有操作留痕，无法精确追溯到具体哪条命令——可能来自另一台电脑的 Antigravity 会话给的建议、拿到这台测试执行，也可能就是本机的 Antigravity——**但无论哪条路，源头都是同一个 AI 助手，不深究了**。结论：**"用 AI 助手执行'关闭最近追踪'类操作 = Win+R 历史高危"已由两起同案支撑**。真嫌"最近使用"碍事，只关**显示**开关即可，别碰**跟踪**开关。
