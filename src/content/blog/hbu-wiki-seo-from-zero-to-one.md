...
- **关于域名切换与 URL 美化（cleanUrls）时的注意事项**：
  - **域名级别**：Vercount 是严格按请求域名（`Hostname`）作为隔离 key 独立记账的。若发生根域名与 `www` 域名的主辅切换，需在代码中将 API 请求 URL 显式映射回历史累计标识，以无缝继承历史积累的 UV/PV 资产。
  - **单页级别**：若开启 `cleanUrls: true` 去除了 `.html` 扩展名，Vercount 默认会将去后缀的新 URL 视为全新页面。此时需在组件上报前自动规范化补齐 `.html`，即可瞬间找回并继续累加该单页的历史阅读量。
