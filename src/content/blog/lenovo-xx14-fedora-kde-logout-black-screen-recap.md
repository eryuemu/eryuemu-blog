...
其实修复当晚，面对"这些折腾要不要还原"的疑问，得到的答复是：**都不影响系统，不用还原**——删掉的 `~/.pam_environment` 本来就不存在，`~/.cache/plasma*` 会自动重建，GRUB 参数单次生效重启即失效，开机目标已改回 `graphical.target`，甚至 sddm "本来就是出厂默认、无需换回"。听起来系统已经是标准官方状态，没有任何残留。

但第二天回看整个过程，疑虑还是产生了：为了修这个 bug，系统跟着 AI 动了太多地方——装了 sddm 全家桶、改了开机默认目标、删过缓存和配置文件，其中真正对修复有用的只有对齐 kscreenlocker 的那一条升级命令，其余大多是试错的副产品。"无害"不等于"该留着"，疑虑里还有一个具体问题：sddm 是排查时装上去的，现在系统好了，它还需要换回去吗？

于是推翻"不用还原"的结论，定下一条还原原则：**除修 bug 必需的改动外，其余全部还原**。带着这个疑问逐项核对（这次的复盘阵型是两台电脑两套 AI 接力：游戏本跑 Antigravity 交叉验证结论，小新 14 本机跑 Trae 执行核对与还原），结果发现 Gemini 收尾时给出的两个结论本身就不成立，还原范围因此比预想的更大。

先把结论摆出来——所谓"还原"，实际动手的只有两组命令：**卸载 sddm 六包并切回出厂 plasmalogin**（见 9.1），以及**重装被连坐删除的 66 个 KDE PIM 包**（见 9.3）。除此之外，前期排查的那些命令**全部无需还原**，原因分三类：

- **单次生效，重启即失效**：GRUB 按 `e` 加的 `3`、`plymouth.enable=0`、删 `rhgb quiet` 都写在内存里不落盘；`systemctl restart sddm`、`plymouth quit`、`chvt`、`startplasma-wayland` 这类运行时操作同理，不留持久改动；
- **删除的对象本就不存在或可再生**：`~/.pam_environment` 从未配置过，`rm -f` 等于空操作；`~/.cache/plasma*` 是临时缓存，每次进桌面自动重建；
- **已在修复流程中归位**：排查时临时切的 `multi-user.target`，在验证修复时已经 `set-default graphical.target` 改回，无需再动。

| 改动 | 性质 | 处置 |
|---|---|---|
| sddm 六包 + display-manager 指向 | 排查期引入的外来组件 | **卸载，切回出厂 plasmalogin** |
| 63 个被连坐删除的 KDE PIM 包 | 误伤 | **重装（66 包）** |
| GRUB 参数 / 服务重启 / chvt / plymouth | 单次生效或运行时操作 | 不用还原 |
| 删除的缓存与环境变量文件 | 不存在或自动重建 | 不用还原 |
| multi-user.target 临时改动 | 修复时已改回 graphical.target | 不用还原 |
| kwin/kscreenlocker 对齐 + 1730 包全量更新 | 修复本体 | 保留 |
