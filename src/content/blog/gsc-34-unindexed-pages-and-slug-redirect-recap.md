...
updatedDate: '2026-09-13T14:20:10+08:00'
category: '开发'
type: 'ai-organized'
---
...
>
> ⚠️ **2026-09-13 订正**：上面第 1 项里的 `vercel.json` 重定向**当时并没有生效**
> （写法与后加的 `trailingSlash: true` 冲突，3 个旧地址一直返回 404），
> 已于 2026-09-13 真正修复并线上逐一验证 → 详见 **2.1 ④**。

### 🎯 6 类索引状态分类与处理总览（3 + 3 架构）

| 分类定位 | GSC 状态分类 | 影响数量 | 核心根因 | 落地处理方案 / 状态 |
| :--- | :--- | :---: | :--- | :--- |
| **🛠️ 主动治理项 1** | **未找到 (404)** | 1 | 8/12 重构时精简 slug 遗留死链 | ⚠️ **订正：当时配的 301 并未生效**，旧地址一直是 404；**2026-09-13 才真正修好** ✅ → 见 **2.1 ④** |
...
> 🔴 **注意：上面这句话是当时的"预期"，实际上并没有发生** ——
> 这 3 条重定向当日就没有生效，旧地址一直是 404。原因与修复见下面的 **④ 订正**。
> （原文保留不改，作为"预期 ≠ 验证"的现场证据。）

#### ④ ⚠️ 订正（2026-09-13）：上面这条 301 当时**根本没生效**

> **本篇发布 20 天后复核发现：上面配的 3 条重定向，从加上那天起就没有生效过。**
> 线上请求任何一个旧地址，返回的都是 **404** —— 也就是说，当时以为已经修好的 GSC「未找到 (404)」，
> **实际上一直挂着**，而且此后每一次爬虫回访都会再撞一次。

**验证过程**：直接对线上旧地址发请求（并用查询串绕开边缘缓存，排除缓存因素）：

| 请求的旧地址 | 当时实际返回 |
| :--- | :--- |
| `/blog/sennagi-save-mod-and-script-reverse-engineering/` | ❌ 404 |
| `/blog/yukoku-translation-tech-retrospective/` | ❌ 404 |
| `/blog/shui-long-yin-shanghai-galonly/` | ❌ 404 |

**根因：`trailingSlash: true` 与 `:path*` 写法打架。**

上面那份 `vercel.json` 里最初**只有** `redirects`；后来为了实现 §2.3 的尾斜杠收敛，又给同一个文件加上了 `"trailingSlash": true`。开启尾斜杠强跳后，Vercel 会**先把请求规范化成带尾斜杠**、再去匹配重定向规则，于是 `:path*` 这种写法**永远匹配不上** —— 规则形同不存在，而且不会报任何错。

**对照实验**（一次部署、三种写法同测）：

| `source` 写法 | 结果 |
| :--- | :--- |
| `/blog/xxx/`（精确路径 + 尾斜杠） | ✅ **308 → 新地址** |
| `/blog/xxx/:path*/`（尾斜杠写在 `*` **之后**） | ✅ **308 → 新地址** |
| `/blog/xxx/:path*`（**本文原来的写法**） | ❌ **404，完全不匹配** |

**修复（2026-09-13）**：3 条全部改为精确路径写法；同时给当天改名的壁纸篇补上第 4 条。改之前先确认 3 个**跳转目标页**均返回 200，避免"修好跳转却跳到 404"。
...
```json
{
	...
	"redirects": [
		{
			"source": "/blog/sennagi-save-mod-and-script-reverse-engineering/",
			"destination": "/blog/sennaagi-save-script-recap/",
			"permanent": true
		},
		{
			"source": "/blog/yukoku-translation-tech-retrospective/",
			"destination": "/blog/youketsu-localization-recap/",
			"permanent": true
		},
		{
			"source": "/blog/shui-long-yin-shanghai-galonly/",
			"destination": "/blog/shuilongyin-galonly-essay/",
			"permanent": true
		}
	]
...
| 旧地址 | 结果 |
| :--- | :--- |
| `sennagi-save-mod-and-script-reverse-engineering/` | ✅ 308 → `/blog/sennaagi-save-script-recap/` → 最终 200 |
| `yukoku-translation-tech-retrospective/` | ✅ 308 → `/blog/youketsu-localization-recap/` → 最终 200 |
| `shui-long-yin-shanghai-galonly/` | ✅ 308 → `/blog/shuilongyin-galonly-essay/` → 最终 200 |
| `gnome-live-wallpaper-engine-two-bugs-fix-recap/` | ✅ 308 → `/blog/gnome-live-wallpaper-engine-fix-recap/` → 最终 200 |

> 📌 **顺带纠正一个用词**：Vercel 的 `permanent: true` 返回的是 **308**，不是 301。
> 308 是 301 的现代等价物（同样表示永久重定向，且保留请求方法），Google 一并按永久重定向处理。
> 本文标题与正文里的"301"严格说应写作 308 —— **语义结论不变**，但排查时按 308 去对日志才不会看错。

> ⚠️ **真正的教训**：原文写的是"部署后……**将**直接返回 301" ——
> 这是一个**将来时**的**预期**，不是一条**验证记录**。
> "看不见的配置"（重定向、`vercel.json`、环境变量）最危险的地方在于：
> **它会安静地失效，而你以为它已经工作了几十天。**
> 结论：**加完必须真去请求一次**，并把响应码记下来。
...
* 🔴 **补一条（2026-09-13 血的教训）：配完必须实测，光配不算数。**
  * 本文 §2.1 的那 3 条重定向**配了 20 天，也失效了 20 天** —— 因为写法（`:path*`）与后来加的 `trailingSlash: true` 冲突，规则静默失效、不报任何错。
  * 正确写法：**精确路径 + 尾斜杠**，即 `"source": "/blog/旧slug/"`（若要连子路径一起兜，用 `/blog/旧slug/:path*/`，注意尾斜杠在 `*` **之后**）。
  * **验收标准不是"配置写好了"，而是 `curl -I 旧地址` 能看到 308、且跟到底落在 200。**
  * 同类风险：凡是"看不见的配置"（重定向 / 头部 / 环境变量 / 域名绑定），都不要用"预期"代替"验证"。
