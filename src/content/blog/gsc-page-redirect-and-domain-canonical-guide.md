...
### 2.3 破案细节：为什么 7 月 20 日就绑定了域名，却恰好是这 3 个被收录？

很多站长会有疑问：“我的博客早在 **7 月 20 日** 就正式绑定了自定义域名并上线，为什么 GSC 面板里既不是全部收录、也不是全部重定向，而是精准地‘收录了 3 个，重定向了 4 个’？而且这 7 个页面为什么会在 8 月 14 日同时跳出来？”
...
| 网址（URL） | 上次抓取时间 | Google 最终判定状态 | 抓取路径与成因分析 |
| :--- | :--- | :--- | :--- |
| `https://eryuemu.com/blog/knowledge-base-and-blog-setup/` | **7 月 27 日**（绑定后仅第 7 天） | 网页会自动重定向 | 域名配置期撞上 308 跳转 |
| `https://eryuemu.com/blog/` | **8 月 05 日** | 网页会自动重定向 | 域名配置期撞上 308 跳转 |
| `https://eryuemu.com/about/` | **8 月 06 日** | 网页会自动重定向 | 域名配置期撞上 308 跳转 |
| `https://eryuemu.com/blog/hbu-wiki-dev-env-setup/` | **8 月 08 日** | 网页会自动重定向 | 域名配置期撞上 308 跳转 |
| `https://eryuemu.com/thoughts/2026-08-05-ai-and-curiosity/` | **8 月 10 日** | **✅ 成功编入索引** | 顺着首页内链抓取，无跳转阻碍 |
| `https://eryuemu.com/`（博客首页） | **8 月 11 日** | **✅ 成功编入索引** | GitHub 外链引流入站，首批直入核心库 |
| `https://eryuemu.com/thoughts/`（心迹列表） | **8 月 15 日** | **✅ 成功编入索引** | 首页重点内链，回访直接收录 |
...
- **路径 A（成功收录通道）：GitHub 外链引导与首页内链**
  ...
  2. Googlebot 的外链探测爬虫在 8 月 11 日通过外链顺藤摸瓜访问了 **首页**。
  3. 爬虫进入首页后，紧接着爬取了首页重点展示的「心迹」模块及当时最新的动态（8 月 5 日的 `2026-08-05-ai-and-curiosity`）。
  4. 当时这几个页面没有触发复杂的子域跳转，页面结构与原创内容完整，Googlebot 便将其作为首批核心页面直接写入了 `eryuemu.com` 的正式索引库。

- **路径 B（重定向记录通道）：7 月 20 日绑定后的域名调整期**
  1. 7 月 20 日域名绑定至 8 月初期间，正值配置 DNS 解析与 Vercel 自定义域名阶段。
  2. 在此期间，Vercel 开启了将 Apex 根域名向 `www` 的自动 308 跳转，或者当时 Astro 的 trailing slash（末尾斜杠）产生了路由规范化。
  3. 爬虫在这一阶段（7 月 27 日首次光顾）爬取 `/blog/...` 与 `/about/` 时，每次请求都收到了服务器返回的 308 重定向响应，因此被真实记录为了「网页会自动重定向」。
...
- 在此之前，Google 底层爬虫实际上已经在后台默默抓取并记录了半个多月。
